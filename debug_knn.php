...
try {
    ...
    if ($result) {
        ...
        echo "<table border='1' style='border-collapse: collapse; width: 100%;'>";
        echo "<tr><th>Rank</th><th>Helm</th><th>Class</th><th>Manhattan Distance</th><th>Original Data</th><th>Normalized Data</th></tr>";
        ...
        foreach (array_slice($result['all_distances'], 0, 10) as $index => $item) {
            ...
            // Calculate individual feature differences (absolute values)
            $k1_diff = abs($input_norm['harga'] - $normalized['harga']);
            $k6_diff = abs($input_norm['berat'] - $normalized['berat']);
            $k3_diff = abs($input_norm['kaca'] - $normalized['kaca']);
            $k4_diff = abs($input_norm['double_visor'] - $normalized['double_visor']);
            $k5_diff = abs($input_norm['ventilasi_udara'] - $normalized['ventilasi_udara']);
            $k7_diff = abs($input_norm['wire_lock'] - $normalized['wire_lock']);
            $k2_diff = abs($input_norm['standar'] - $normalized['standar']);
        ...
        echo "</table>";
        echo "<p style='font-size: 11px; color: #666;'><strong>Keterangan:</strong> K1=Harga, K2=Standar, K3=Kaca, K4=Double Visor, K5=Ventilasi, K6=Berat, K7=Wire Lock (selisih absolut dari input vs training)</p>";
        ...
        if ($id63_item) {
            ...
            $diff = abs($input_norm['berat'] - $norm63['berat']);
            
            echo "<strong>Absolute Difference:</strong> " . number_format($diff, 15) . "<br>";
        ...
        echo "<table border='1' style='border-collapse: collapse; width: 100%; font-size: 11px;'>";
        echo "<tr style='background: #f8f9fa;'>";
        echo "<th>No</th><th>Berat Asli</th><th>Berat Norm</th><th>Input Norm</th><th>Abs Diff</th>";
        echo "</tr>";
        
        foreach (array_slice($result['all_distances'], 0, 10) as $index => $item) {
            $data = $item['data'];
            $normalized = $item['normalized'];
            $diff = abs($input_norm['berat'] - $normalized['berat']);
    ...
    if (isset($_POST['predict'])) {
        ...
        try {
            ...
            if ($form_result) {
                ...
                echo "<strong>Manhattan Distance Formula:</strong><br>";
